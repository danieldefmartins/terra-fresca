...
export function createPortTransfer(T, original, environment) {
  ...
  return {draw(renderer,time,width,height,film){
    ...
    rails.forEach((m,i)=>{m.position.set(end.x+(i?1:-1)*span*.62,120,dockZ+reach/2);m.scale.set(5,9,reach)});
    cross.forEach((m,i)=>{m.position.set(end.x,120,dockZ+(i?reach:0));m.scale.set(span*1.3,9,5)});
    trolley.position.set(x,119,z);trolley.scale.set(span*1.27,9,12);
    ...
    cables.forEach((m,i)=>segment(m,new T.Vector3(x+(i%2?1:-1)*span*.55,160,z),new T.Vector3(x+(i%2?1:-1)*size*.9,spreader.position.y,z+(i<2?1:-1)*size*5.2),.9));
    braces.forEach((m,i)=>{const side=i<8?-1:1,j=i%8,rx=end.x+side*span*.62;segment(m,new T.Vector3(rx-3,120,dockZ+j*reach/8),new T.Vector3(rx+3,120,dockZ+(j+1)*reach/8),1.3)});
    ...
    return {lift,traverse,lower,land,landingError:Math.hypot(x-end.x,z-end.z)};
  }};
}
